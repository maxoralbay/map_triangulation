...
class Router
{
    ...
    public function run()
    {
        ...
        $staticFileExtensions = ['css', 'js', 'png', 'jpg', 'jpeg', 'gif', 'svg', 'ico', 'woff', 'woff2', 'ttf', 'map'];
        $urlParts = explode('.', parse_url($this->url, PHP_URL_PATH));
        $fileExtension = end($urlParts);

        if (in_array($fileExtension, $staticFileExtensions)) {
            // Serve the static file directly
            $filePath = PUBLIC_DIR . $this->url; // Adjust PUBLIC_DIR to your public directory path
